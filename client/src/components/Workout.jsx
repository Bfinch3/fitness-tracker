...
function Workout({ title, type, text, links, comments }) {
  ...
  useEffect(() => {
    titleInputField.current.value = editTitle;
    workoutTypeSelection.current.value = editType;
    textDiv.current.innerHTML = markdownConverter.makeHtml(editText);
  }, []);
  ...
  async function onSave() {
    ...
    // ---- If the user has edited, for example, the text, it will be the edited text ------//
    // editTitle;
    // editType;
    // editText;
    // editLinks;
  ...
  return (
    <div className="card flex-grow-1 box-shadow col-7">
      ...
      <div className="card-body">
        ...
        <WorkoutLinks inEditMode={inEditMode} links={links} editLinks={editLinks} setEditLinks={setEditLinks}/>
